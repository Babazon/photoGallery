...
server.patch('/api/images/:imageId/comments/:commentId', (req: Request, res: Response) => {
    ...
    const existingComment: CommentDto|undefined = image.comments.find((c: CommentDto) => c.id === commentId);
  ...
  server.delete('/api/images/:imageId/comments/:commentId', (req: Request, res: Response) => {
    const { imageId, commentId } = req.params;
    ...
    const commentIndex = image.comments.findIndex((c: CommentDto) => c.id === commentId);
    if (commentIndex === -1) {
      res.sendStatus(404);
      return;
    }
